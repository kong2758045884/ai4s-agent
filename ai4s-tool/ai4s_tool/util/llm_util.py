...
from ai4s_tool.util.sensitive_detection import SensitiveWordsReplace
...
def _prepare_litellm_params(model: str, **kwargs: Any) -> dict:
    ...
    elif explicit_dashscope or (
        not explicit_api_base
        and model.startswith("deepseek-")
        and env_openai_api_base
        and _is_dashscope_api_base(env_openai_api_base)
    ):
        # 百炼也提供 DeepSeek；工具未显式传地址时复用全局百炼配置。
        use_dashscope = True
        api_base_raw = explicit_api_base or env_openai_api_base
