...
- **来源标注**：如果参考资料提供了引用编号与“引用链接”，你必须直接输出 Markdown 链接格式的引用，例如 `[〔1〕](https://example.com/source-1)`；如果一个段落来自多个参考资料，请依次列出多个 Markdown 引用；严禁编造、改写或省略参考资料中给出的引用链接；只有在参考资料没有提供链接时，才允许退化为纯文本 `〔1〕`。
...
- **引用必须可点击**：所有带链接的来源标注都必须输出成 Markdown 链接，禁止输出不可点击的 `〔1〕〔2〕` 纯文本编号。
