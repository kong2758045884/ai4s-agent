...
from ai4s_tool.tool.docread._compat import SyncTool, ToolCategory, ToolContext, ValidationResult
...
class ExcelReaderTool(SyncTool):
    ...
    def execute_sync(self, params: dict[str, Any], context: ToolContext) -> dict[str, Any]:
        ...
        if suffix == ".xls":
            raise ValueError(
                f"Legacy .xls is not supported by excel_reader: {file_path}. "
                "Convert to .xlsx first (Excel Save As / LibreOffice), then retry."
            )
        ...
        # .xlsx/.xlsm are ZIP packages; corrupt downloads often fail with
        ...
        if head[:2] == b"PK":
            ...
            raise ValueError(
                f"File looks like legacy OLE Excel (.xls) but extension is {suffix}: {file_path}. "
                "Re-save as real .xlsx and retry."
            )
        elif head.lstrip().startswith((b"<", b"<!DOCTYPE", b"{", b"HTTP")):
            raise ValueError(
                f"File is not Excel binary (looks like HTML/JSON/text): {file_path}. "
                "Check upload/download path; file may be an error page renamed to .xlsx."
            )
        else:
            raise ValueError(
                f"File does not look like a valid .xlsx ZIP package: {file_path} "
                f"(size={size}, magic={head[:4]!r}). Re-upload a valid .xlsx."
            )
        ...
        try:
            ...
            raise ValueError(
                f"Invalid or corrupted Excel file: {file_path}. "
                "Re-export as .xlsx or re-upload the original file."
            ) from e
        except Exception as e:
            msg = str(e)
            if "central directory" in msg.lower() or "bad offset" in msg.lower() or "zip" in msg.lower():
                raise ValueError(
                    f"Excel package is truncated/corrupt (ZIP central directory): {file_path} "
                    f"(size={size}). Common causes: incomplete download, wrong path, "
                    "or non-xlsx content with .xlsx extension. Re-upload a complete .xlsx."
                ) from e
