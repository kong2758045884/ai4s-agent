...
from ai4s_tool.tool.analysis_component.schema_data import get_data
from ai4s_tool.tool.analysis_component.data_model import (
    DataModel,
    Measure,
    SiblingGroup,
)
from ai4s_tool.tool.analysis_component.insights import (
    InsightFactoryDict,
    InsightType,
)
...
class AnalysisDataFetchError(RuntimeError):
    """取数失败后终止本轮分析，避免模型继续编造替代数据。"""
...
class GetDataTool(Tool):
    ...
    def _record_data_fetch_failure(self, message: str) -> None:
        self.context.data_fetch_error = message
    ...
    def forward(self, query: str) -> pd.DataFrame:
        ...
        if not isinstance(query, str) or not query.strip():
            message = "get_data 的 query 必须是非空字符串，无法完成分析"
            self._record_data_fetch_failure(message)
            raise AnalysisDataFetchError(message)
        ...
        try:
            ...
            if isinstance(exc, Timeout):
                message = f"取数超时，无法完成分析：{exc}"
            else:
                message = f"取数请求失败，无法完成分析：{exc}"
            self._record_data_fetch_failure(message)
            raise AnalysisDataFetchError(message) from exc
        if isinstance(base_datas, list):
            if not base_datas:
                message = "取数返回空结果，无法完成分析"
                self._record_data_fetch_failure(message)
                raise AnalysisDataFetchError(message)
            ...
            return df
        # 非列表结果通常代表权限、接口或参数错误，不能作为有效分析数据继续传递。
        message = f"取数未返回有效数据，无法完成分析：{base_datas}"
        self._record_data_fetch_failure(message)
        raise AnalysisDataFetchError(message)
    ...
    def merge_df(cls, datas: List[Dict]) -> pd.DataFrame:
        ...
        if len(datas) > 1 and (
            prefix := "_".join(
                [
                    f"{f['name']}为{f['val']}"
                    for f in datas[0]["filters"]
                    if f["opt"] in ["EQUALS"]
                    and f["dataType"] in ["VARCHAR"]
                    and not re.match(r"^\d{4}-\d{2}-\d{2}$", f["val"])
                ]
            )
        ):
            joined_df = joined_df.rename(
                {
                    ...
                    if c["guid"] in datas[0]["measureCols"]
                },
                axis=1,
            )
        on_cols = [
            c["name"] for c in datas[0]["columnList"] if c["col"] in datas[0]["dimCols"]
        ]
        for data in datas[1:]:
...
class InsightTool(Tool):
    ...
    inputs = {
        "df": {"type": "object", "description": "分析的数据，必须是 pd.DataFrame 类型"},
        "breakdown": {
            "type": "string",
    ...
    def forward(
        ...
        insight: InsightType = InsightFactoryDict[analysis_method].from_data(
            sibling_group
        )
        return (
            insight.model_dump(
...
class SaveInsightTool(Tool):
    ...
    def forward(
        self, df: pd.DataFrame, insight: str, analysis_process: str
    ) -> pd.DataFrame:
        ...
        # 补充异常情况处理
            ...
            logger.error(
                f"{RequestIdCtx.request_id} {self.name} df type[{type(df)}] is not pd.DataFrame"
            )
            data = ""
        return self.context.save_insight(
            df=data, insight=insights, analysis_process=analysis_process
        )
...
class DataTransTool(Tool):
    ...
    def forward(
        ...
        if trans_type == "rank":
            ...
            df = df.groupby(column).agg({measure: agg}).reset_index()
            df[f"Rank({measure})"] = (
                df[measure].rank(ascending=False, method="dense").astype(int)
            )
        return df
