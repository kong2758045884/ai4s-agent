...
from ai4s_tool.tool.table_rag.utils import read_json
...
class ColumnFilterModule:
    ...
    def __init__(
        ...
        self.table_filter_model_name = (
            os.getenv("TR_COLUMN_FILTER_MODEL_NAME") or _default
        )

        self._is_first_filter_table = os.getenv("TR_IS_FIRST_FILTER_TABLE", True)
        self.need_filter_table_min_length = int(
            os.getenv("TR_NEED_FILTER_TABLE_MIN_LENGTH", 3)
        )
        ...
        self.business_prompt_max_length = int(
            os.getenv("TR_BUSINESS_PROMPT_MAX_LENGTH", 3000)
        )
    ...
    def time_info(self) -> str:
        ...
        if self.current_date_info == "" or self.current_date_info == " ":
            self.current_date_info = (
                f"今天是 {today.strftime('%Y年%m月%d日')}，星期{day_of_week}"
            )
    ...
    def _generate_table_filter_prompt(
        self, table_schema_info_list, error_msg: str | None
    ):
        table_info_list_str = ""
        ...
        for index, table_schema_info in enumerate(table_schema_info_list):
            ...
            # 仅保留部分关键信息用于选表
            # 字段名，字段内容，fewshot，
            schema_list_str = self.schema_list2str(schemaList)[
                : self.schema_list_max_length
            ]
    ...
    async def _filter_single_table(
        self, semaphore, table_schema_info: dict
    ) -> dict | None:
        ...
        async with semaphore:
            ...
            for retry in range(3):
                try:
                    # 每次重试必须只解析本次响应，避免把多个 JSON 响应拼接成非法内容。
                    llm_response = ""
                    columns_prompt = self._generate_filter_prompt(
                        table_schema_info, error_msg
                    )
                    ...
                    if str(result_dict["relatedFlag"]).lower() == "true":
                        ...
                        for column_info in columns:
                            ...
                            if (
                                column_index in result_column_indexes
                                or default_recall == 1
                            ):
                    ...
                    traceback.print_exc()
                    logger.error(
                        f"[filter column] {request_id}, fail to filter columns error_msg {error_msg}"
                    )
                    continue

            raise RuntimeError(
                f"[filter column] {request_id} 多次重试后执行失败, error_msg:{error_msg}"
            )
    ...
    async def batch_get_stage_result(self):
            ...
            try:
                ...
                for item in filtered_batch:
                    task = asyncio.create_task(
                        self._filter_single_table(semaphore2, item)
                    )
        ...
        filter_tables = await asyncio.gather(
            *second_stage_tasks, return_exceptions=True
        )
    ...
    async def filter_table(self, semaphore, schema_info_list):
        ...
        async with semaphore:
            ...
            model_code_list = [
                table_schema_info.get("modelCode", "")
                for table_schema_info in schema_info_list
            ]
            logger.info(
                f"sn: {request_id} Start [filter tables] tables: {model_code_list}"
            )
            prompt, index2model_code_map = self._generate_table_filter_prompt(
                schema_info_list, error_msg
            )
            ...
            llm_response = llm_response[
                llm_response.find("[") : llm_response.find("]") + 1
            ]

            # 开始选表
            model_code_index_list = read_json(llm_response)
            schema_info_list = [
                index2model_code_map[index] for index in model_code_index_list
            ]
            model_code_list = [
                table_schema_info.get("modelCode", "")
                for table_schema_info in schema_info_list
    ...
    async def batch_get_result(self):
        ...
        if (
            len(table_schema_lists) < self.need_filter_table_min_length
            or not self._is_first_filter_table
        ):
            semaphore = asyncio.Semaphore(self.table_filter_batch_size)
            filter_tables = await asyncio.gather(
                *[
                    self._filter_single_table(semaphore, item)
                    for item in table_schema_lists
                ]
            )
...
async def main():
    body = {
        ...
        "dbType": "h2",
        "modelCodeList": ["sales", "sales_count"],
        "query": "不同国家的数量",
