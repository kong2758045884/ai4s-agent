...
from ai4s_tool.tool.table_rag.retriever import Retriever


class ElasticsearchClientAuthTest(unittest.TestCase):

    def test_should_use_api_key_for_cloud_es(self):
        with patch("ai4s_tool.tool.table_rag.es_client.Elasticsearch") as elasticsearch_cls:
            ElasticsearchClient(
                {
    ...
    def test_retriever_should_read_data_agent_es_config_only(self):
        with patch.dict(
            os.environ,
            {
                ...
                "TR_ES_CONFIGS_INDEX": "ai4s_model_column_value",
            },
            clear=True,
        ):
            with patch("ai4s_tool.tool.table_rag.retriever.ElasticsearchClient") as elasticsearch_client_cls:
        ...
        self.assertEqual("ai4s_model_column_value", retriever.es_index)
    ...
    def test_should_retry_without_analyzer_when_ik_plugin_is_missing(self):
        ...
        with patch("ai4s_tool.tool.table_rag.es_client.Elasticsearch", return_value=fake_client):
