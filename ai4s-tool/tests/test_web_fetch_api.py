...
from ai4s_tool.tool.web_fetcher import WebFetchResult
...
class WebFetchApiTest(unittest.TestCase):
    ...
    def test_should_return_unified_response_and_file_info(self):
        ...
        with patch("ai4s_tool.api.tool.WebFetcher.fetch", new=AsyncMock(return_value=fetch_result)):
            with patch(
    ...
    def test_should_return_400_when_fetcher_rejects_request(self):
        with patch("ai4s_tool.api.tool.WebFetcher.fetch", new=AsyncMock(side_effect=ValueError("web_fetch 仅支持 HTML、Markdown 或纯文本内容"))):
            response = self.client.post(
                "/v1/tool/web_fetch",
                json={
