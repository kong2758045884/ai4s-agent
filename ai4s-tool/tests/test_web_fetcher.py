...
from ai4s_tool.model.protocal import WebFetchRequest
from ai4s_tool.tool.web_fetcher import DownloadedPage, WebFetcher
...
class WebFetcherTest(unittest.IsolatedAsyncioTestCase):
    async def test_should_forward_configured_proxy_to_http_request(self):
        ...
        with patch.dict(
            "os.environ",
            {
                ...
                "AI4S_REDDIT_SESSION": "reddit-session-secret",
            },
        ):
            with patch("ai4s_tool.tool.web_fetcher.aiohttp.ClientSession", FakeSession):
    ...
    async def test_should_extract_markdown_content_with_trafilatura(self):
        ...
        with patch.object(fetcher, "_download_page", new=AsyncMock(return_value=page)):
            with patch(
                "ai4s_tool.tool.web_fetcher.trafilatura.extract",
                return_value="# 标题\n\n正文内容",
            ):
    ...
    async def test_should_fallback_to_beautifulsoup_when_trafilatura_returns_empty(self):
        ...
        with patch.object(fetcher, "_download_page", new=AsyncMock(return_value=page)):
            with patch("ai4s_tool.tool.web_fetcher.trafilatura.extract", return_value=""):
                result = await fetcher.fetch(request)
