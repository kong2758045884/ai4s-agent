...
| `ExecutionLedgerReadRepository` | AI4S 执行账本读仓储实现 |
| `ExecutionLedgerWriteRepository` | AI4S 执行账本写仓储实现 |
...
### AI4S Phase 2A 持久化边界
- AI4S ledger、tool-output、chat-model DAO / Mapper 统一位于 `src/main/java/org/wwz/ai/infrastructure/dao/ai4s/`
...
### Agent DDD 最终边界（019）
- `OkHttpRemoteHttpAdapter`、`OkHttpRemoteStreamAdapter`、`AI4SToolFileArtifactAdapter` 是 runtime 对外部 HTTP / 文件产物的主适配器
...
- 允许继续引用 `org.wwz.ai.domain.agent.ai4s.config.data.*`，因为 `DataQueryExecutionAdapter`、`DataQueryMetadataAdapter`、`JdbcUtils` 仍需要共享数据库连接配置契约
- 允许继续引用 `org.wwz.ai.domain.agent.ai4s.model.imagegeneration.*` 与 `org.wwz.ai.domain.agent.ai4s.service.IWorkspaceImageGenerationService` / `org.wwz.ai.domain.agent.ai4s.service.imagegeneration.*`，因为工作台生图入口与技术执行器仍共用这组稳定历史契约
