...
export default function StrategicMap() {
  ...
  const [selectedTeamId, setSelectedTeamId] = useState(DOMAINS[0]?.teams[0]?.id ?? "");
  ...
  const selectedTeam =
    visibleTeams.find((team) => team.id === selectedTeamId) ?? visibleTeams[0] ?? EMPTY_TEAM;
    ...
    loadStrategicMap()
      .then((snapshot) => {
        ...
        if (nextDomains.length) {
          ...
          const selectedDomain = nextDomains.find((domain) => domain.id === activeDomainId) ?? nextDomains[0];
          setActiveDomainId(selectedDomain.id);
          setSelectedTeamId(selectedDomain.teams[0]?.id ?? "");
        }
  ...
  const selectDomain = (domain: Domain) => {
    ...
    setActiveSubdomainId("");
    setSelectedTeamId(domain.teams[0]?.id ?? "");
  };
  ...
  const refreshTeams = async () => {
    ...
    setError("");
    try {
      const result = await loadStrategicDomainTeams(activeDomain.id, { refresh: true });
      setDomains((current) => current.map((domain) => (
        domain.id === activeDomain.id
          ? { ...domain, ...(result.domain ?? {}), teams: result.teams }
          : domain
      )));
      setSource(result.source);
      const nextSubdomainId = activeSubdomainId && result.domain?.subdomains.some((item) => item.id === activeSubdomainId)
        ? activeSubdomainId
        : "";
      if (nextSubdomainId !== activeSubdomainId) setActiveSubdomainId(nextSubdomainId);
      const nextVisible = nextSubdomainId
        ? result.teams.filter((team) => team.subdomainId === nextSubdomainId)
        : result.teams;
      setSelectedTeamId(rankCandidateTeams(nextVisible)[0]?.id ?? "");
    } catch (reason) {
  ...
  const saveEditor = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editor.kind === "domain") {
        ...
        if (!editor.id) setActiveSubdomainId(next.id);
      }
      setEditor(null);
    } catch (reason) {
  ...
  const removeSubdomain = async (subdomain: StrategicSubdomain) => {
    ...
    try {
      ...
      setDomains((current) => current.map((domain) => domain.id === activeDomain.id
        ? { ...domain, subdomains: domain.subdomains.filter((item) => item.id !== subdomain.id) }
        : domain));
      if (activeSubdomainId === subdomain.id) setActiveSubdomainId("");
    } catch (reason) {
  ...
  return (
    <div className="flex h-full min-h-0 w-full flex-col overflow-hidden bg-[#f3f6f9] text-[var(--chat-text)]">
      ...
      <div className="mx-auto flex min-h-0 w-full max-w-[1600px] flex-1 flex-col gap-3 overflow-y-auto p-3 sm:gap-4 sm:p-4 md:p-5 lg:overflow-hidden lg:flex-row">
        <aside
        ...
        >
          ...
          <div className={leftNavCollapsed ? "hidden" : "flex min-h-0 flex-1 flex-col px-1 pr-1"}>
            <div className="min-h-0 h-[72%] shrink-0 overflow-y-auto pr-1">
              {domains.map((domain) => {
            ...
            <div className="flex min-h-0 h-[28%] shrink-0 flex-col border-t border-[#e4eaf1] pt-3">
              ...
              <div className="min-h-0 flex-1 overflow-y-auto pr-1">
                <button type="button" onClick={() => { setActiveSubdomainId(""); setSelectedTeamId(activeDomain.teams[0]?.id ?? ""); }} className={`mt-2 flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-[13px] ${!activeSubdomainId ? "bg-[#eef6fb] font-semibold text-[#236ca8]" : "text-[#607486] hover:bg-[#f5f8fb]"}`}><DomainIcon label={activeDomain.label} className="size-3.5 shrink-0" /><span>全部子领域</span></button>
                {activeDomain.subdomains.map((subdomain) => (
                  <div key={subdomain.id} className="group/sub relative mt-1 flex items-center">
                    <button type="button" onClick={() => { setActiveSubdomainId(subdomain.id); setSelectedTeamId(activeDomain.teams.find((team) => team.subdomainId === subdomain.id)?.id ?? ""); }} className={`flex min-w-0 flex-1 items-center gap-2 rounded-lg px-3 py-2 pr-16 text-left text-[13px] ${activeSubdomainId === subdomain.id ? "bg-[#eef6fb] font-semibold text-[#236ca8]" : "text-[#607486] hover:bg-[#f5f8fb]"}`}><DomainIcon label={activeDomain.label} className="size-3.5 shrink-0" /><span className="block truncate">{subdomain.name}</span></button>
        ...
        <main className="flex min-h-[420px] min-w-0 flex-none flex-col rounded-xl border border-[#d1dce7] bg-white p-3.5 shadow-[0_4px_14px_rgba(27,64,96,0.05)] sm:p-4 md:p-5 lg:min-h-0 lg:flex-1">
          <div className="flex flex-wrap items-start justify-between gap-3 border-b border-[#e6edf4] pb-4">
            ...
            <button type="button" onClick={() => void refreshTeams()} disabled={syncing || !activeDomain.id || activeDomain.id === EMPTY_DOMAIN.id} className="inline-flex shrink-0 items-center gap-1.5 rounded-full border border-[#cbd8e5] px-3 py-1.5 text-[12px] font-medium text-[#5f7181] transition hover:border-[#6fa1c2] hover:text-[#23648f] disabled:cursor-not-allowed disabled:opacity-60" title="从 AI4S Daily 获取当前领域最新候选">
              {syncing ? <LoaderCircle className="size-3.5 animate-spin" /> : <RefreshCcw className="size-3.5" />}
              {syncing ? "研判中…" : "重新研判"}
            </button>
          </div>

          <div className="flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto pr-1 pt-4">
                ...
                >
                  ...
                  <span className="flex min-w-0 items-center gap-3">
                    ...
                    <span className="min-w-0" aria-label={`${teamOrganization(team)} · ${teamDisplayName(team)}`}>
                      <span className="block truncate text-[15px] font-semibold text-[#274158]">
                        {teamOrganization(team)}
                      </span>
                      <span className="mt-1 block truncate text-[12px] font-medium text-[#6a8194]">
        ...
        <aside className="flex min-h-[380px] w-full shrink-0 flex-col overflow-hidden rounded-xl border border-[#d1dce7] bg-white p-3.5 shadow-[0_4px_14px_rgba(27,64,96,0.05)] sm:p-4 md:p-5 lg:min-h-0 lg:w-[430px]">
          ...
          <div className={`min-h-0 flex-1 ${teamEditing ? "overflow-y-auto pr-1" : "overflow-hidden"}`}>
