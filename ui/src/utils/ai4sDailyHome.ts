...
请先调用 ai4s_daily，检索该热点及其主题相关的历史 AI4S Daily 报告和热点内容；再根据证据是否充分、问题范围和时效性，决定是否继续使用 DeepSearch 或 WebSearch 补充论文、官网、GitHub、预印本和其他一手资料。不要因为首页摘要已有内容就跳过 ai4s_daily，也不要把 AI4S Daily 当作唯一知识源。至少覆盖 3 类来源：原始论文/预印本、机构或项目官网、代码仓库或数据集（主题确实存在时）。每条关键事实记录标题、机构/作者、日期、URL 和对应段落。

在综合和生成正式 HTML 前，先加载并遵守 "ai4s-report-analysis" skill；它规定证据分级、去重、章节职责和内部错误隔离。
...
最终请形成一份事实密度高、逻辑递进的专题研究，至少包括：
1. 事件概览：时间线、触发事件、参与机构和可核验数字
2. 科学问题：研究对象、关键假设、评价指标和仍未解决的约束
3. 核心技术路线：模型/算法、数据、训练或实验流程、工程栈与输入输出
4. 主要创新：逐条对应来源事实，说明相对前序工作的变化
5. 论文、团队和机构：作者、团队、机构、代表成果及其关系
6. 相关前序工作：时间顺序、继承关系和证据链接
7. 同类研究或竞争路线：国内外路线的目标、方法、数据、公开结果和差异
8. 对 AI4S 的意义：只根据前述事实推导具体影响，避免空泛口号
9. 后续值得关注的问题：可验证的技术/数据/产业信号、观察方法和时间窗口
10. 来源证据：集中列出所有引用，保留 AI4S Daily 报告 URL、热点原始来源 URL，以及后续搜索证据 URL

章节要求：每章至少给出 2—4 条具体事实或来源；每个判断紧跟证据编号；章节之间不要重复同一段摘要。若某章没有足够证据，明确写资料缺口，不用套话填充。`;
}
